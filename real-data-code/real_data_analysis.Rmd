...
pnt <- readRDS("result\\est.res.pt.rds")
...
ggplot(pnt.SANN.long, aes(x = `MAF Interval`, y = log10(est.), color = Model)) + geom_point(alpha = 0.3) + geom_errorbar(aes(ymin = log10(lower), ymax = log10(upper)), alpha = 0.3) +
  ...
  scale_color_brewer(palette = "Set1") + labs(x = "MAF interval", y = "Point estimate (log10 scale)")
ggsave("Fig.5.png", width = 7.5, height = 5, units = "in")
```
